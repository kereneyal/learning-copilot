              ...
              <div className="h-[520px] overflow-y-auto rounded-2xl border bg-slate-50 p-4">
                {messages.length === 0 ? (
                  <p className="text-sm text-slate-500">
                    ...
                    <br />
                    מצב "אוטומטי" ינסה לזהות לבד את הקורס.
                  </p>
                ) : (
                        ...
                        <div className={`max-w-[85%] rounded-2xl px-4 py-3 shadow-sm ${m.role === "user" ? "bg-blue-600 text-white" : "border border-slate-200 bg-white text-slate-900"}`}>
                          ...
                          {m.sources && m.sources.length > 0 && (
                            <div className="mt-3 border-t pt-3">
                              ...
                              <div className="space-y-2">
                                {m.sources.map((s, idx) => (
                                  <div key={idx} className="rounded-lg bg-slate-50 p-2 text-xs text-slate-600">
                                    <div>course: {s.course_id || "n/a"}</div>
                                    <div>lecture: {s.lecture_id || "n/a"}</div>
                                    <div>document: {s.document_id || "n/a"}</div>
