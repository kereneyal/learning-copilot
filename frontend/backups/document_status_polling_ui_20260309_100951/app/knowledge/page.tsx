...
export default function KnowledgePage() {
    ...
    try {
      ...
      const successCount =
        Array.isArray(data?.results)
          ? data.results.filter((r: any) => r.processing_status === "ready").length
          : 0

      setToast({
        message: `העלאה הושלמה. ${successCount} קבצים מוכנים לעיבוד/הושלמו.`,
        type: "success",
      })
  ...
  async function deleteCourse(courseId: string) {
    if (!window.confirm("למחוק את הקורס? פעולה זו תמחק גם הרצאות, מסמכים וידע קשור.")) return
    try {
  ...
  async function deleteLecture(lectureId: string) {
    if (!window.confirm("למחוק את ההרצאה? פעולה זו תמחק גם מסמכים קשורים.")) return
    try {
