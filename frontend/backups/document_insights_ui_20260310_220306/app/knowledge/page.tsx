    ...
    try {
      const res = await fetch(`${API_BASE}/documents/${editingDocument.id}`, {
        ...
        body: JSON.stringify({
          ...
          lecture_id: editingDocument.lecture_id || null,
        }),
      })
      if (!res.ok) throw new Error()
      ...
      {editingLecture && (
        <Modal title="עריכת הרצאה" onClose={() => setEditingLecture(null)}>
          <div className="space-y-3">
            ...
            <select className="w-full rounded-xl border px-3 py-2" value={editingLecture.lecturer_id || ""} onChange={(e) => setEditingLecture({ ...editingLecture, lecturer_id: e.target.value || null })}>
              <option value="">בחר מרצה</option>
              {lecturers.map((lecturer) => (
                <option key={lecturer.id} value={lecturer.id}>
                  {lecturer.full_name}
      ...
      {editingDocument && (
        <Modal title="עריכת מסמך" onClose={() => setEditingDocument(null)}>
          <div className="space-y-3">
            ...
            <select className="w-full rounded-xl border px-3 py-2" value={editingDocument.source_type || "slides"} onChange={(e) => setEditingDocument({ ...editingDocument, source_type: e.target.value })}>
              <option value="slides">מצגת</option>
              <option value="summary">סיכום</option>
              <option value="notes">הערות</option>
              <option value="article">מאמר</option>
            ...
            <select className="w-full rounded-xl border px-3 py-2" value={editingDocument.lecture_id || ""} onChange={(e) => setEditingDocument({ ...editingDocument, lecture_id: e.target.value })}>
              <option value="">ללא הרצאה</option>
              {lectures.map((lecture) => (
                <option key={lecture.id} value={lecture.id}>
                  {lecture.title}
                </option>
