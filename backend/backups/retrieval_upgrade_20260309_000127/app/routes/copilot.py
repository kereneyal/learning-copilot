...
def build_qa_prompt(question: str, context: str, language: str):
    if language == "he":
        return f"""
...
ענה בעברית, בצורה ברורה, קצרה ומעשית.
"""
    else:
        return f"""
...
Answer clearly, practically, and in English.
"""


def hybrid_retrieve(course_id: str, question: str):
    ...
    if not chunks:
        ...
        if keyword_results and keyword_results.get("documents"):
            docs = keyword_results["documents"][0]
            metas = keyword_results["metadatas"][0]
            for doc, meta in zip(docs, metas):
                chunks.append(doc)
                sources.append(meta)
...
def build_prompt(intent: str, course_id: str, question: str, language: str, db: Session):
    if intent == "qa":
        chunks, sources = hybrid_retrieve(course_id, question)
        context = "\n\n".join(chunks[:6])
        prompt = build_qa_prompt(question, context, language)
